...
デフォルトでは Off です。必要な場合だけ `--auto_rename` を付けると、変更前のファイル名とOCR結果の先頭4ページ・末尾4ページを併せて判定し、元文書を次の形式に自動変更します。変更前のファイル名に日付、文書種類、証拠番号、表題などが含まれる場合は候補情報として使い、OCR本文と矛盾する場合は本文を優先します。判定には全ページ数と、PDFの場合は各ページの用紙サイズ（実寸mm）も渡します。数十ページある文書をちらし扱いにしたり、名刺やパンフレットを複数まとめたPDFを先頭1件の表題で呼んだりしないためです。
...
- 文書種類は固定候補から選びます（全ページ数と用紙サイズも判断材料になります）
